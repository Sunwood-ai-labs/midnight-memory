# 前奏 / 後奏 split subtitle フロー

main lyric cue の前後に未カバーの vocal がある場合、このリポジトリでは 1 つの字幕に押し込まず `intro` / `main` / `outro` の split subtitle として管理します。
...
LTX セグメントも一緒に確認したい場合は `timelineSubtitles` を追加します。
