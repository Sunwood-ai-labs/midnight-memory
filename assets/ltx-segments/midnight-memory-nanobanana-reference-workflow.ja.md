...
- まずはロケーション固定マスター1枚につき、カメラ差分だけを変えた 1 枚を出す
- 候補を増やしたいときは、同じプロンプトをこちらで何回も実行して集める
...
- まずは各ロケーション固定マスターごとに、カメラ位置、寄り引き、顔の向き、視線だけを変えた 1 枚を出す
- その中から「一番よい1枚」を選んで、本採用カットにする
- つまり、ロケーションの世界観は master に閉じ込めて、ショット側ではカメラ調整だけに寄せる

## Candidate Generation Policy

- 候補生成段階では、表情や衣装や背景を大きく変えない
- ランダムにしてよいのは、カメラ角度、距離、寄り引き、視線、わずかな首の向き程度
- 1回で 1 枚だけ出す。必要なら同じプロンプトをこちらで何回も回して候補を集め、その中から採用カットを選ぶ
- 採用後に必要なら、そこから微修正を1回だけ入れる
