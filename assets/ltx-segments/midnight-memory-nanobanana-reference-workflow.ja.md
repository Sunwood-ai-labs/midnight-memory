...
- まずはロケーション固定マスター1枚につき、カメラ差分だけを変えた複数候補を出して、その中から採用カットを選ぶ
...
- まずは各ロケーション固定マスターごとに、カメラ位置、寄り引き、顔の向き、視線だけを変えた複数候補をまとめて出す
...
- 1回で複数候補を出し、その中から採用カットを選ぶ。枚数は固定しない。必要なら 10 枚前後を目安にしてよい
