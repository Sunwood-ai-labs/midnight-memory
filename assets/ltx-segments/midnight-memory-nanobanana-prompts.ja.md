...
4. 最終ショットは `画像1 = 主役`, `画像2 = ロケーション固定マスター` で、まず 10 枚前後の候補を出す
5. 候補から1枚選び、必要ならその1枚だけ微修正する
...
- 候補生成段階では、カメラ角度、距離、寄り引き、視線だけをランダムに振る
...
このセクションが、あなたの言う「ロケーション固定マスター画像だけにして、カメラのアングルや距離をランダムにして10枚くらい作る」ための汎用プロンプトです。
...
同じキャラクター、同じ衣装、同じ場所、同じ空気感を維持したまま、カメラの角度、距離、寄り引き、顔の向き、視線だけをランダムに変えた候補を10枚前後作る。  
表情はフラット寄り。口は閉じ気味か少しだけ開く程度。唇と顎のラインは見える。  
背景やロケーションの構造は画像2に従い、大きく変えない。  
主役の同一性は画像1に厳密に従う。  
各候補は、別カメラで撮った同一シーンの静止フレームのように見せる。  
文字なし、ウォーターマークなし。
```
...
Prompt:
```text
画像1の狐娘を主役として固定し、画像2のボーカルブースのロケーション固定マスター画像を参照して生成する。  
同じキャラクター、同じスタジオ、同じ夜景のまま、カメラの角度、距離、寄り引き、顔の向き、視線だけをランダムに変えた候補を10枚前後作る。  
顔アップからミディアムクローズまでの範囲で振る。  
表情はフラット寄り。口元は見える。  
ボーカルブースの構造、窓の夜景、マイク位置は大きく変えない。  
文字なし、ウォーターマークなし。
```
...
Prompt:
```text
画像1の狐娘を主役として固定し、画像2の屋上ステージのロケーション固定マスター画像を参照して生成する。  
同じキャラクター、同じ屋上、同じ夜景のまま、カメラの角度、距離、寄り引き、顔の向き、視線だけをランダムに変えた候補を10枚前後作る。  
顔アップからミディアムクローズまでの範囲で振る。  
表情はフラット寄り。口元は見える。  
屋上ステージの構造、床の反射、夜景の位置は大きく変えない。  
文字なし、ウォーターマークなし。
```
...
Prompt:
```text
画像1の狐娘を主役として固定し、画像2の川沿いブリッジのロケーション固定マスター画像を参照して生成する。  
同じキャラクター、同じブリッジ、同じ水面反射のまま、カメラの角度、距離、寄り引き、顔の向き、視線だけをランダムに変えた候補を10枚前後作る。  
顔アップからミディアムクローズまでの範囲で振る。  
表情はフラット寄り。口元は見える。  
欄干、水面、背景の光は大きく変えない。  
文字なし、ウォーターマークなし。
...
Prompt:
```text
画像1の狐娘を主役として固定し、画像2のガラス越し都会空間のロケーション固定マスター画像を参照して生成する。  
同じキャラクター、同じ都会空間、同じガラス反射のまま、カメラの角度、距離、寄り引き、顔の向き、視線だけをランダムに変えた候補を10枚前後作る。  
顔アップからミディアムクローズまでの範囲で振る。  
表情はフラット寄り。口元は見える。  
ガラスの反射やネオンの滲みは保ちつつ、構造は大きく変えない。  
文字なし、ウォーターマークなし。
```
