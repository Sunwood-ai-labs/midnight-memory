...
このセクションが、ロケーション固定マスター画像だけを参照して、カメラのアングルや距離をランダムに変えた 1 枚を毎回出すための汎用プロンプトです。
...
同じキャラクター、同じ衣装、同じ場所、同じ空気感を維持したまま、カメラの角度、距離、寄り引き、顔の向き、視線だけをランダムに変えた 1 枚を出す。  
表情はフラット寄り。口は閉じ気味か少しだけ開く程度。唇と顎のラインは見える。  
背景やロケーションの構造は画像2に従い、大きく変えない。  
主役の同一性は画像1に厳密に従う。  
毎回、別カメラで撮った同一シーンの静止フレームのように見せる。  
文字なし、ウォーターマークなし。
```
...
同じキャラクター、同じスタジオ、同じ夜景のまま、カメラの角度、距離、寄り引き、顔の向き、視線だけをランダムに変えた 1 枚を出す。  
...
同じキャラクター、同じ屋上、同じ夜景のまま、カメラの角度、距離、寄り引き、顔の向き、視線だけをランダムに変えた 1 枚を出す。  
顔アップからミディアムクローズまでの範囲で振る。  
表情はフラット寄り。口元は見える。  
...
同じキャラクター、同じブリッジ、同じ水面反射のまま、カメラの角度、距離、寄り引き、顔の向き、視線だけをランダムに変えた 1 枚を出す。  
顔アップからミディアムクローズまでの範囲で振る。  
表情はフラット寄り。口元は見える。  
欄干、水面、背景の光は大きく変えない。  
文字なし、ウォーターマークなし。
...
同じキャラクター、同じ都会空間、同じガラス反射のまま、カメラの角度、距離、寄り引き、顔の向き、視線だけをランダムに変えた 1 枚を出す。  
顔アップからミディアムクローズまでの範囲で振る。  
表情はフラット寄り。口元は見える。  
