...
4. 最終ショットは `画像1 = 主役`, `画像2 = ロケーション固定マスター` で、まず複数の候補を出す
...
このセクションが、ロケーション固定マスター画像だけを参照して、カメラのアングルや距離をランダムに変えた複数候補を毎回出すための汎用プロンプトです。
...
同じキャラクター、同じ衣装、同じ場所、同じ空気感を維持したまま、カメラの角度、距離、寄り引き、顔の向き、視線だけをランダムに変えた複数候補を出す。枚数は固定しない。必要なら 10 枚前後を目安にしてよい。  
...
同じキャラクター、同じスタジオ、同じ夜景のまま、カメラの角度、距離、寄り引き、顔の向き、視線だけをランダムに変えた複数候補を出す。枚数は固定しない。必要なら 10 枚前後を目安にしてよい。  
...
同じキャラクター、同じ屋上、同じ夜景のまま、カメラの角度、距離、寄り引き、顔の向き、視線だけをランダムに変えた複数候補を出す。枚数は固定しない。必要なら 10 枚前後を目安にしてよい。  
顔アップからミディアムクローズまでの範囲で振る。  
表情はフラット寄り。口元は見える。  
...
同じキャラクター、同じブリッジ、同じ水面反射のまま、カメラの角度、距離、寄り引き、顔の向き、視線だけをランダムに変えた複数候補を出す。枚数は固定しない。必要なら 10 枚前後を目安にしてよい。  
顔アップからミディアムクローズまでの範囲で振る。  
表情はフラット寄り。口元は見える。  
欄干、水面、背景の光は大きく変えない。  
文字なし、ウォーターマークなし。
...
同じキャラクター、同じ都会空間、同じガラス反射のまま、カメラの角度、距離、寄り引き、顔の向き、視線だけをランダムに変えた複数候補を出す。枚数は固定しない。必要なら 10 枚前後を目安にしてよい。  
顔アップからミディアムクローズまでの範囲で振る。  
表情はフラット寄り。口元は見える。  
