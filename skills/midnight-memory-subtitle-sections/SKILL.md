...
description: Split `midnight-memory` track subtitles into `intro`, `main`, and `outro` parts, verify extra vocal regions with Gemini, keep combined `.srt` files and `assets/manifest.json` synchronized, and preserve metadata for viewer-specific timeline treatment. Use when Codex needs to extract 前奏 or 後奏 cues, create `.intro.srt` / `.main.srt` / `.outro.srt` files, update `subtitles` arrays, or maintain this repo's segmented subtitle workflow.
---
...
Read [references/workflow.md](./references/workflow.md) when you need the full command sequence, naming rules, and validation checklist.
...
3. Probe the uncovered clip with `uv run python scripts/gemini_srt.py --allow-extra-text` and targeted Gemini 3 trims.
4. Keep wording conservative:
   - use stable lyrics or ad-lib tokens when models converge
   - use a neutral placeholder only when the wording stays ambiguous
5. Create or refresh:
